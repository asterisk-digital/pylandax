...
def test_all_document_functions():
    ...
    if bool(get_documents) == True:
        ...
        if get_document_content.status_code == 200:
            documents_upload = client.upload_document(
                get_document_content.content, f"test-{get_documents[0]['FileName']}", pylandax_test_folder_id
            )
            print(f"Documents upload: {documents_upload}\n")

            if documents_upload.status_code == 200:
                push_document_content = client.push_document_content(
                    get_document_content.content, int(get_documents[0]["Id"])
                )
                print(f"Document push content: {push_document_content}\n")

                if push_document_content.status_code == 200:
                    delete_document = client.delete_data(
                        "Documents", documents_upload.json()["value"]["document"]["DocumentId"]
                    )
